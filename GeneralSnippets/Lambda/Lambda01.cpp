...
namespace Lambdas {
    ...
    static void test_03()
    {
        ...
        std::sort(
            vec.begin(),
            vec.end(),
            LocalComparer{ false }  // LocalComparer{ false } // Was ist das in dieser Zeile: LAMBDA
        );
    ...
    static void test_04() {
        ...
        std::sort(
            vec.begin(),
            vec.end(),
            [=](int n1, int n2) {         // soll flag_direction als KOPIE oder als ALIAS (Referenz) verwendet werden ?

                std::println("Lambda: vgl. {} mit {}", n1, n2);
                return (flag_direction) ? n1 < n2 : n1 > n2;
                // return  n1 < n2;
            }
        );
    ...
    static void test_07_a() {

        // lambda with variable definition
        auto lambda = [variable = 111]() mutable  -> int  {

            /*static auto variable = 111;*/
            variable++;
            return variable;
        };
        ...
        std::cout << lambda() << std::endl; //  112
        std::cout << lambda() << std::endl; //  113
        std::cout << lambda() << std::endl; //  114

        std::cout << lambda2() << std::endl; // 115
